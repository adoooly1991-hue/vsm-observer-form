...
if st.button("Generate observations"):
    ...
    st.session_state['vsm_result'] = result
    st.session_state['vsm_steps'] = steps
    st.session_state['vsm_templates'] = templates
...
if st.button("Export PPTX"):
    ...
    templates_ss = st.session_state.get("vsm_templates", templates)
    result_ss = st.session_state.get("vsm_result", None)
    obs_ss = st.session_state.get("vsm_obs", None)

    if result_ss is None:
        result_ss = compute_lead_time(steps_ss, available_time_sec=available_time_hr*3600.0)
        st.session_state["vsm_result"] = result_ss
    if obs_ss is None:
        # Build observations quickly if missing
        import pandas as pd
        obs_rows = []
        id_to_name = {s.id: s.name for s in steps_ss}
        for idx, s in enumerate(steps_ss):
            w = score_wastes(s, templates_ss["thresholds"])
            ctx = {"prev_name": id_to_name.get(steps_ss[idx-1].id) if idx>0 else None,
                   "waiting_sec": result_ss["by_step"].get(s.id,{}).get("waiting_sec",0.0)}
            for waste in ["defects","waiting","inventory","overproduction","transportation","motion","overprocessing","talent"]:
                row = make_observation(s, waste, w, templates_ss, templates_ss["thresholds"], ctx)
                if row: obs_rows.append(row)
        obs_ss = pd.DataFrame(obs_rows)
        if not obs_ss.empty:
            obs_ss = obs_ss.sort_values(["rpn_pct","score_0_5"], ascending=False).reset_index(drop=True)
        st.session_state["vsm_obs"] = obs_ss
    ...
    # Effective CT map for spacing
    ct_eff_map = {sid: result_ss["by_step"].get(sid, {}).get("ct_eff_sec", 0.0) for sid in result_ss["by_step"].keys()}
